          ...
          <View style={[styles.sectionCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <SettingItem
              ...
              onValueChange={(val: any) => {
                const boolVal = typeof val === 'boolean' ? val : !!val?.target?.checked;
                updateSettings({ notifications: boolVal });
              }}
            />
            ...
            <SettingItem
              ...
              onValueChange={(val: any) => {
                const boolVal = typeof val === 'boolean' ? val : !!val?.target?.checked;
                updateSettings({ theme: boolVal ? 'dark' : 'light' });
              }}
            />
            <View style={[styles.divider, { backgroundColor: colors.divider }]} />
            ...
            <View style={styles.fontSizeContainer}>
              <View style={styles.themeHeader}>
                ...
                <View style={styles.settingTextContainer}>
                  ...
                  <Text style={[styles.settingSubtitle, { color: colors.subtitle, fontSize: Math.max(10, settings.fontSize * 0.75) }]}>Arraste para ajustar a leitura</Text>
                </View>
              </View>
                ...
                <Slider
                  ...
                  onValueChange={(val) => {
                    ...
                    if (typeof val === 'number') {
                      ...
                      // Attempt to extract value if it is an event object
                      const eventVal = (val as any).nativeEvent?.value || (val as any).target?.value;
                      if (!isNaN(Number(eventVal))) {
                        num = Number(eventVal);
                      }
                    }
                    updateSettings({ fontSize: num });
                  }}
